...
"""
Reve Pretraining Script with YAML and CLI Configuration Overrides.

This script provides a flexible way to pretrain Reve models using Megatron-Bridge with support for
both YAML configuration files and command-line overrides using Hydra-style syntax.

Examples:
    Basic usage with default configuration:
        $ torchrun --nproc_per_node=8 pretrain_reve.py

    Using a custom YAML config file:
        $ torchrun --nproc_per_node=8 pretrain_reve.py --config-file my_custom_config.yaml

    Using CLI overrides only:
        $ torchrun --nproc_per_node=8 pretrain_reve.py model.tensor_model_parallel_size=4 train.train_iters=100000

    Combining YAML and CLI overrides (CLI takes precedence):
        $ torchrun --nproc_per_node=8 pretrain_reve.py --config-file conf/my_config.yaml \
...
from dfm.src.megatron.model.reve.reve_step import ReveForwardStep
from dfm.src.megatron.recipes.reve.reve import pretrain_config
...
# Assumes this script (pretrain_reve.py) is in Megatron-Bridge/examples/recipes/reve/
# and the config is in a 'conf' subdirectory.
SCRIPT_DIR: Path = Path(__file__).parent.resolve()
DEFAULT_CONFIG_FILENAME: str = "reve_pretrain_override_example.yaml"
...
def parse_cli_args() -> Tuple[argparse.Namespace, list[str]]:
    ...
    parser = argparse.ArgumentParser(
        description="Pretrain Reve model using Megatron-Bridge with YAML and CLI overrides",
        formatter_class=argparse.RawTextHelpFormatter,
    )
    parser.add_argument("--mock", action="store_true", help="Whether to use mock data.")
    parser.add_argument("--model-size", choices=["small", "full", "full_dimhead128", "1b"], default="small", help="Set model size, 'small', 'full', 'full_dimhead128', or '1b'.")
    parser.add_argument(
        "--training-mode",
    ...
    parser.add_argument(
        ...
        help="Path to the YAML OmegaConf override file. Default: conf/reve_pretrain_override_example.yaml",
    )
...
def main() -> None:
    """
    Entry point for the Reve pretraining script.
    ...
    Examples of CLI usage:
        ...
        torchrun --nproc_per_node=8 pretrain_reve.py optimizer.lr=0.0002

        # Custom config file with additional overrides
        torchrun --nproc_per_node=8 pretrain_reve.py --config-file my_config.yaml train.train_iters=50000
    ...
    logger.info("Megatron-Bridge Reve Pretraining Script with YAML & CLI Overrides")
    logger.info("------------------------------------------------------------------")

    # Load base configuration from the recipe as a Python dataclass
    cfg: ConfigContainer = pretrain_config(mock=args.mock, model_size=args.model_size)
    ...
    if args.training_mode == "pretrain":
        reve_forward_step = ReveForwardStep(
    ...
    pretrain(config=cfg, forward_step_func=reve_forward_step)
