...
def _sparse_attn_fwd(
    ...
    K_block_ptr = K + qvk_offset + (offs_n[None, :] // BLOCK_N) * stride_kn + offs_headsize[:, None] * stride_kk
    V_block_ptr = V + qvk_offset + (offs_n[:, None] // BLOCK_N) * stride_vk + offs_headsize[None, :] * stride_vn
    O_block_ptr = Out + qvk_offset + offs_m[:, None] * stride_om + offs_headsize[None, :] * stride_on
    O_accum_block_ptr = Out_accum + qvk_offset + offs_m[:, None] * stride_om + offs_headsize[None, :] * stride_on
    # initialize offsets
    ...
    acc, l_i, m_i = _sparse_attn_fwd_inner(
        acc,
        l_i,
        m_i,
        q,
...
class _sparse_attention(torch.autograd.Function):
    ...
    def forward(ctx, q, k, v, o_accum, sm_scale, sparsity_indices, sparsity_counts, O_scale=1.0):
        ...
        _sparse_attn_fwd[grid](
            ...
            o.stride(2),
            o.stride(3),  #
            sparsity_indices.stride(0),
            sparsity_indices.stride(1),  #
            sparsity_counts.stride(0),
            sparsity_counts.stride(1),  #
            q.shape[0],
            q.shape[1],  #
            N_CTX=q.shape[2],  #
            HEAD_DIM=HEAD_DIM_K,  #
...
def get_sparsity_data(b, h, seqlen, sparsity_amount=0.5):
    ...
    blockmask = rearrange(blockmask.unsqueeze(3).expand(-1, -1, -1, BLOCK_SIZE_M, -1), "b h mb bm n -> b h (mb bm) n")[
        :, :, :seqlen
    ]
    ...
    if provider == "flash":
        if key not in baseline_results:
            ...
            ans = triton.testing.do_bench(
                lambda: F.scaled_dot_product_attention(q, k, v, attn_mask=None, dropout_p=0.0, is_causal=False),
                quantiles=quantiles,
            )
        ...
        results = triton.testing.do_bench(
            lambda: tl_dense(q, k, v, False, 0.5), quantiles=quantiles, warmup=100, rep=1000
        )
        ...
        results = triton.testing.do_bench(
            lambda: sparse_attention(q, k, v, o_accum, 0.5, sp_ind, sp_counts),
            quantiles=quantiles,
            warmup=100,
            rep=1000,
        )
