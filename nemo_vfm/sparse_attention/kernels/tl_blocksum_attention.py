...
def _full_attn_fwd_inner(
    ...
    off_h = off_hb % H
    softmax_data_offset = (
        off_b.to(tl.int64) * softmax_stride_b
        + off_h.to(tl.int64) * softmax_stride_h
        + (start_m * BLOCK_M + tl.arange(0, BLOCK_M)) * softmax_stride_n
    )
    ...
    prev_normalization_final = tl.load(
        prev_normalization_final_ptrs + softmax_data_offset, mask=(offs_m < seqlen), other=1.0e6
    )
...
def _full_attn_fwd(
    ...
    #     order=(1, 0),
    # )
    offs_o = (start_m * BLOCK_M + tl.arange(0, BLOCK_M)[:, None]) * stride_om + tl.arange(0, HEAD_DIM)[
        None, :
    ] * stride_on
...
@triton.testing.perf_report(
    [
        triton.testing.Benchmark(
            ...
            ylabel="Kernel Duration (% of FlashAttention)",
            plot_name="FlashAttention Performance",
        )
    ]
)
def benchmark_blocksum_attn(sparsity_amt, provider):
    ...
    rescale = lambda tuple: (
        tuple[0] / baseline_results[key][0],
        tuple[1] / baseline_results[key][1],
        tuple[2] / baseline_results[key][2],
    )
    ...
    if provider == "flash":
        if key not in baseline_results:
            ...
            ans = triton.testing.do_bench(
                lambda: F.scaled_dot_product_attention(q, k, v, is_causal=False), quantiles=quantiles
            )
            baseline_results[key] = ans
        return rescale(baseline_results[key])
    elif provider == "blocksum":
        results = triton.testing.do_bench(
            lambda: full_attention(q, k, v, 0.5, prev_maxes, prev_normalization),
            quantiles=quantiles,
            warmup=100,
            rep=1000,
        )
...
if __name__ == "__main__":
    ...
    prev_m = (
        torch.empty((Z, H, N_CTX), dtype=torch.float32, device=DEVICE).normal_(mean=0.0, std=0.5).requires_grad_(False)
    )
    prev_l = (
        torch.empty((Z, H, N_CTX), dtype=torch.float32, device=DEVICE)
        .normal_(mean=0.0, std=0.5)
        .abs()
    ...
    assert (ref_out - tri_out).abs().max().item() < 0.01, (
        "The output of the reference attention implementation and the tri implementation are not close enough."
    )
    print("Max m difference:", (tri_m - ref_m).abs().max().item())
    assert (tri_m - ref_m).abs().max().item() < 0.02, (
        "The m values of the reference attention implementation and the tri implementation are not close enough."
    )
    ...
    # breakpoint()
    assert (tri_blocksums - ref_blocksums).abs().mean().item() < 0.7, (
        "The blocksums of the reference attention implementation and the tri implementation are not close enough."
    )
