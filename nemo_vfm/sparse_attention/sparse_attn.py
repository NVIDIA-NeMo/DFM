...
def flash_attn_func(q, k, v, *args, **kwargs):
    if q.shape != k.shape or q.shape != v.shape: return original_flash_attn_forward(q, k, v, *args, **kwargs)
    kwargs['should_rearrange'] = True
    o = invoke_sparse_layer(q, k, v, *args, **kwargs)
    if o is None: return original_flash_attn_forward(q, k, v, *args, **kwargs)
    return (o, None, None, None, o, q[:, :, :, 0], None, None)

def flash_attn_varlen_func(q, k, v, *args, **kwargs):
    if q.shape != k.shape or q.shape != v.shape: return original_flash_attn_varlen_forward(q, k, v, *args, **kwargs)
    kwargs['should_rearrange'] = True
    o = invoke_sparse_layer(q, k, v, *args, should_rearrange=True, **kwargs)
    if o is None: return original_flash_attn_varlen_forward(q, k, v, *args, **kwargs)
    return (o, None, None, None, o, q[:, :, :, 0], None, None)

def sdpa_func(q, k, v, *args, **kwargs):
    if q.shape != k.shape or q.shape != v.shape: return original_sdpa(q, k, v, *args, **kwargs)
    o = invoke_sparse_layer(q, k, v, *args, should_rearrange=False, **kwargs)
    if o is None: return original_sdpa(q, k, v, *args, **kwargs)
    return o
...
def setup_sparse_attn(file_path):
    ...
    sparse_attn_layers = [[SparseDiffAttn(i, layer_counter) for j in range(layer_counter.num_submodules_per_layer)] for i in range(layer_counter.num_layers)]    
...
def invoke_sparse_layer(q, k, v, *args, should_rearrange, **kwargs):
    ...
    w, h, d = GLOBAL_CONFIG['model_config']['latent_vector_shape']['width'], GLOBAL_CONFIG['model_config']['latent_vector_shape']['height'], GLOBAL_CONFIG['model_config']['latent_vector_shape']['depth']
    ...
    if not has_initialized:
        sparse_attn_layers[0][0].initialize_static_mask(
            seq_shape=(w, h, d),
            txt_len=0,
            local_heads_num=num_heads // tensor_parallel_size,
            device='cuda'
        )
