...
class WanT2VTrainer:
    ...
    def train(
        ...
        if is_main_process():
            ...
            if WANDB_AVAILABLE:
                wandb.init(
                    project=f"wan-t2v-overfit-video-{self.mode}",
                    config=config,
                    resume=resume_checkpoint is not None,
                )
            else:
                print0("[INFO] wandb not available - skipping logging to wandb")
        ...
        for epoch in range(start_epoch, num_epochs):
            ...
            for microbatch_idx, batch in enumerate(iterable):
                ...
                if not is_accumulating:
                    # FSDP-aware gradient clipping (computes global norm across all ranks)
                    fsdp_model = self.model_map["transformer"]["fsdp_transformer"]
                    grad_norm = fsdp_model.clip_grad_norm_(self.grad_clip)
