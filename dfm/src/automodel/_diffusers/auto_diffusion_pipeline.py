...
def _apply_fsdp2_parallelization(
    pipe,
    parallel_scheme: Optional[Dict[str, Dict[str, Any]]],
) -> Dict[str, FSDP2Manager]:
    """Apply FSDP2 parallelization to pipeline components."""
    created_managers: Dict[str, FSDP2Manager] = {}
    if parallel_scheme is None:
        return created_managers

    assert torch.distributed.is_initialized(), "Distributed environment must be initialized for FSDP2"
    _init_parallelizer()
    ...
    for comp_name, comp_module in _iter_pipeline_modules(pipe):
        ...
        if manager_args is None:
            continue
        logger.info("[INFO] Applying FSDP2 to %s with args: %s", comp_name, manager_args)
...
class NeMoAutoDiffusionPipeline:
    ...
    def from_pretrained(
        ...
        **kwargs,
        ...
        Returns:
            Tuple of (DiffusionPipeline, Dict[str, FSDP2Manager])
        """
        ...
        # Apply FSDP2 parallelization
        created_managers = _apply_fsdp2_parallelization(pipe, parallel_scheme)
    ...
    def from_config(
        ...
        components_to_load: Optional[Iterable[str]] = None,
        **kwargs,
    ) -> Tuple["NeMoAutoDiffusionPipeline", Dict[str, FSDP2Manager]]:
        """
        ...
        Returns:
            Tuple of (NeMoAutoDiffusionPipeline, Dict[str, FSDP2Manager])
        """
