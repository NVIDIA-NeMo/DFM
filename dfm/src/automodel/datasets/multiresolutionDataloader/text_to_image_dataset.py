...
class TextToImageDataset(Dataset):
    ...
    def _group_by_bucket(self):
        ...
        self.sorted_bucket_keys = sorted(
            self.bucket_groups.keys(),
            key=lambda k: self.bucket_groups[k]['pixels']
        )
        ...
        for key in self.sorted_bucket_keys:
            ...
            logger.info(f"  {aspect_name:6s} {resolution[0]:4d}x{resolution[1]:4d}: "
                  f"{len(bucket['indices']):5d} samples")
    ...
    def get_bucket_info(self) -> Dict:
        ...
        return {
            ...
            'buckets': {
                f"{k[0]}/{k[1][0]}x{k[1][1]}": len(v['indices'])
