...
class MultiTierBucketCalculator:
    ...
    def get_bucket_for_image(
        ...
        best_bucket = min(
            self.buckets,
            key=lambda b: abs(b['aspect_ratio'] - image_aspect)
        )
    ...
    def resize_and_crop(
        ...
        if crop_mode == 'center':
            ...
            left = max(0, min(new_width - target_width, 
                             center_x - target_width // 2 + random.randint(-50, 50)))
            top = max(0, min(new_height - target_height,
                            center_y - target_height // 2 + random.randint(-50, 50)))
    ...
    def from_preset(cls, preset: str, quantization: int = 64) -> 'MultiTierBucketCalculator':
        ...
        if preset not in cls.RESOLUTION_PRESETS:
            raise ValueError(
                ...
                f"Available: {list(cls.RESOLUTION_PRESETS.keys())}"
            )
