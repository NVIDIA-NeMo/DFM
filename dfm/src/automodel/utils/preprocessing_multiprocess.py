...
def _process_image(args: Tuple) -> Optional[Dict]:
    ...
    image_path, output_dir, verify, caption_field = args
    ...
    try:
        ...
        caption = _load_caption(image_path_obj, caption_field) or image_path_obj.stem.replace("_", " ")
...
def _process_shard_on_gpu(
    ...
    for image_path in tqdm(image_files, desc=f"GPU {gpu_id}", position=gpu_id):
        result = _process_image((str(image_path), output_dir, verify, caption_field))
        if result:
    ...
    with Pool(processes=num_gpus) as pool:
        args = [
            (gpu_id, chunks[gpu_id], str(output_dir), processor_name, 
             model_name, verify, caption_field, max_pixels)
