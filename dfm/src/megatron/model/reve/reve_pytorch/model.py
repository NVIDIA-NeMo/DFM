...
class ReveV2(nn.Module):
    ...
    def forward(
        ...
        y = torch.randn(
            bs, num_tokens, txt_token_dim, dtype=dtype, device=device
        )
        ...
        # DEBUGGING
        if torch.distributed.get_rank() == 0:
            print(f"[DEBUG] (reve_pytorch/model.py) x: {x.shape}")
            print(f"[DEBUG] (reve_pytorch/model.py) x_position_ids: {x_position_ids.shape}")
            print(f"[DEBUG] (reve_pytorch/model.py) timestep: {timestep.shape}")
            print(f"[DEBUG] (reve_pytorch/model.py) y: {y.shape}")
            print(f"[DEBUG] (reve_pytorch/model.py) y_mask: {y_mask.shape}")
            print(f"[DEBUG] (reve_pytorch/model.py) conditioning_signal: {conditioning_signal.shape}")
        ...
        if torch.distributed.get_rank() == 0:
            print(f"[DEBUG] (reve_pytorch/model.py) patched_img.shape - patched_img.mean() - patched_img.std(): {patched_img.shape} - {patched_img.mean()} - {patched_img.std()}")
            print(f"[DEBUG] (reve_pytorch/model.py) --------------------------------")
