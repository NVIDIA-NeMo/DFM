...
class FlowPipeline:
    ...
    def training_step(
        ...
        number_packed_samples = data_batch["number_packed_samples"]
        video_latents = data_batch["video_latents"]
        x_position_ids = data_batch["x_position_ids"]
        context_embeddings = data_batch["context_embeddings"]
        text_seq_len = data_batch["text_seq_len"]
        loss_mask = data_batch["loss_mask"]
        conditioning_signal = data_batch["conditioning_signal"]
        grid_sizes = data_batch["grid_sizes"]
        packed_seq_params = data_batch["packed_seq_params"]
        text_packed_seq_params = data_batch["text_packed_seq_params"]
        ...
        if parallel_state.is_pipeline_last_stage():
            model_pred = self.model(
                ...
                text_packed_seq_params=text_packed_seq_params,
            )
