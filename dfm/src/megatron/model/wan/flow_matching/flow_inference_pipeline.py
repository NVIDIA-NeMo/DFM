...
class FlowInferencePipeline:
    ...
    def generate(
        ...
        if initial_latents is not None:
            ...
            # This allows teacher to start from the same noise as student
        ...
        with amp.autocast(dtype=self.param_dtype), torch.no_grad(), no_sync():
            ...
            for _, t in enumerate(tqdm(timesteps)):
                ...
                # latents = []
                if add_pre_type_x0:
                    # For x0 prediction (single-step distilled model):
                    # The model output IS the final x0 - no scheduler step needed
                    pass
                else:
                    # For flow prediction (multi-step teacher):
                    # Use scheduler to step from current latent towards x0
                    latents = schedulers[0].step(latents, t, unpatchified_latents, return_dict=False)[
                        0
                    ]  # Extract the prev_sample tensor from the tuple
