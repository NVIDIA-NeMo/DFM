...
def mock_batch(
    ...
    video_latent = torch.randn(in_channels, F_latents, H_latents, W_latents, dtype=torch.float32)
    ...
    # conditional signal
    conditioning_signal = torch.tensor([0.7], dtype=torch.bfloat16)

    # For Reve model's position ids
    text_seq_len = context_seq_len
    x_position_ids = torch.ones(number_packed_samples, seq_len_q, 3, dtype= torch.bfloat16)
...
class ReveMockDataModuleConfig(DatasetProvider):
    path: str = ""
    seq_length: int
