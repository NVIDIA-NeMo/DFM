...
from Automodel.fsdp.dfm_fsdp import DFMFSDPManager
...
class NeMoAutoDiffusionPipeline(DiffusionPipeline):
    ...
    def from_pretrained(
        ...
        *model_args,
        parallel_scheme: Optional[Dict[str, DFMFSDPManager]] = None,
        ...
        if parallel_scheme is not None:
            ...
            for comp_name, comp_module in _iter_pipeline_modules(pipe):
                ...
                if manager is None:
                    continue
                try:
                    new_m = manager.parallelize_diffusion_model(comp_module)
