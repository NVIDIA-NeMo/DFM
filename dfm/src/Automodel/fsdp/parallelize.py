...
def get_parallelization_strategy(model: nn.Module) -> ParallelizationStrategy:
    ...
    strategy = PARALLELIZATION_STRATEGIES.get(model_class_name, DefaultParallelizationStrategy())
    if strategy is None:
        raise ValueError(f"No parallelization strategy found for model class: {model_class_name}")
    return strategy
